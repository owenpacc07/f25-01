...
export async function loadData(user_id, experiment_id, family_id, mechanism_id) {
    ...
    if (response2.output === "No output generated for m005") {
        console.error("No output generated for mechanism m005. Check server-side logic or output file (out-005.dat).");
        return false;
    }
    ...
    if (data.output.length === 0 && data.extraOutput.length === 0) {
        console.warn("Output data is empty. Verify the output format from out-005.dat.");
    }
