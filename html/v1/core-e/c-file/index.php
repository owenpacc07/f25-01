...
$coreEPath = realpath("../../../files/core-e/c-file/");
$inputFile = "$path/in-file.dat";
...
/*if (isset($_POST['save_results'])) {
    // Existing save_results code updated to use new output file
    $filenameIN = "$path/in-$mid.dat"; // Still used for experiment storage
    $fileSizes = isset($_POST['file_sizes']) ? $_POST['file_sizes'] : '';
    $newDataIN = $fileSizes;
    file_put_contents($filenameIN, $newDataIN);

    $filenameOUT = "$path/out-$mid.dat"; // Still used for experiment storage
    ...
    $restrict_view = 1;
    $compare = "file"; // Fixed potential issue with 'file' as a variable
    $codeFilePath = realpath("../../../cgi-bin/core-e/e-$compare/");
...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['compare'])) {
        ...
        if (!file_exists($coreEPath)) {
            mkdir($coreEPath, 0770, true);
        }
