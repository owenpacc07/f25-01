...
function updateDirectory(mfile) {
    ...
    $('#dir tr:last').after('<tr><td>' + fileName + '</td>' + '<td>' + fileStart + '</td>' + '<td>' + fileLength + '</tr>');
...
function displayFullDirectory(mfile) {
    ...
    for (var i = 0; i < mfile.length; i++) {
        fileName = mfile[i][1];
        fileStart = parseInt(slots.indexOf(mfile[i][0])) + 1;
...
function animate(type) {
    ...
    for (let i = 0; i < numberOfBlocks; i++) {
        ...
        // displays the arrow image on the first block, then displays a blank space
        //
        if (i == 0) {
            blocks += "<td id=arrow><img src=\"arrow.png\" alt=\"Arrow\"><td>";
        } else {
            blocks += "<td " + " style=\"width: 8px; height: 30px;\"><\/td>";
        }
    }
