...
<head>
	...
	function randomizeDiskSchedulingInputs() {
    	let head = Math.floor(Math.random() * 701); // Head position between 0-700
    	let queueSize = Math.floor(Math.random() * 10) + 1; // Between 1-10 numbers in queue
    	let values = new Set();

    	while (values.size < queueSize) {
        	values.add(Math.floor(Math.random() * 699) + 1); // Requests between 1-699
    	}

    	document.getElementById("headInput").value = head;
    	document.getElementById("requestInput").value = Array.from(values).join(", ");
    	document.getElementById("diskSizeInput").value = 700; // Fixed disk size
	}
...
<script type="module" src="load_data.js"></script>
...
<body>
            ...
            <div class="col-md-6">
				...
				<label for="diskSizeInput">Disk Size (comma-separated):</label>
                            ...
                            <button type="button" class="btn btn-purple d-inline mr-2" onclick="showChart(event)">Show
                                Chart</button>
			   <button type="button" class="btn btn-purple d-inline mr-2" onclick="randomizeDiskSchedulingInputs();">Randomize Inputs</button>
	...
	$filePath = realpath("../../../files/core-c/c-disk/in-disk.dat");
        file_put_contents($filePath, $newContents);




	file_put_contents("../../../files/core-c/c-disk/flag-file.txt", "1");
...
*/
	...
	$stringResults = [
  	'FCFS'  => fcfsAlgorithmString($requests, $head, $diskSize),
  	'SSTF'  => sstfAlgorithmString($requests, $head, $diskSize),
  	'CSCAN' => cscanAlgorithmString($requests, $head, $diskSize),
  	'LOOK'  => lookAlgorithmString($requests, $head, $diskSize),
  	'CLOOK' => clookAlgorithmString($requests, $head, $diskSize),
	];
	...
	$filePath = realpath("../../../files/core-c/m-041/out-041.dat");
        $newContents = "$line1\n$line2\n$stringResults[FCFS]";
        file_put_contents($filePath, $newContents);

	$filePath = realpath("../../../files/core-c/m-042/out-042.dat");
        $newContents = "$line1\n$line2\n$stringResults[SSTF]";
        file_put_contents($filePath, $newContents);

	$filePath = realpath("../../../files/core-c/m-043/out-043.dat");
        $newContents = "$line1\n$line2\n$stringResults[CSCAN]";
        file_put_contents($filePath, $newContents);

	$filePath = realpath("../../../files/core-c/m-044/out-044.dat");
        $newContents = "$line1\n$line2\n$stringResults[LOOK]";
        file_put_contents($filePath, $newContents);

	$filePath = realpath("../../../files/core-c/m-045/out-045.dat");
        $newContents = "$line1\n$line2\n$stringResults[CLOOK]";
        file_put_contents($filePath, $newContents);
...
function fcfsAlgorithmString($requests, $head, $diskSize)
{
    $currentPosition = $head;
    $visitOrder = []; // no head, no distance
    ...
    // Return only the space-separated path
    return implode(" ", $visitOrder);
}

function sstfAlgorithmString($requests, $head, $diskSize)
{
    $currentPosition = $head;
    $remaining       = $requests;
    $visitOrder      = [];
    ...
    while (!empty($remaining)) {
        ...
        for ($i = 1; $i < count($remaining); $i++) {
            $dist = abs($remaining[$i] - $currentPosition);
...
function cscanAlgorithmString($requests, $head, $diskSize)
{
    $sorted = bubbleSort($requests);
    $upper  = [];
    $lower  = [];
    ...
    foreach ($sorted as $req) {
        if ($req >= $head) $upper[] = $req;
        else               $lower[] = $req;
    }
    $currentPosition = $head;
    $visitOrder      = [];

    // Serve upper ascending
    foreach ($upper as $req) {
        $currentPosition = $req;
        $visitOrder[]    = $req;
    }
    // Jump to 0 if lower requests exist
    if (!empty($lower)) {
        $currentPosition = 0; // not recording zero in the path
        // Serve lower ascending
        foreach ($lower as $req) {
            $currentPosition = $req;
            $visitOrder[]    = $req;
        }
    }
...
function lookAlgorithmString($requests, $head, $diskSize)
{
    $sorted = bubbleSort($requests);
    $left   = [];
    $right  = [];
    ...
    // Left descending
    $reversedLeft = array_reverse($left);
    foreach ($reversedLeft as $req) {
        $currentPosition = $req;
        $visitOrder[]    = $req;
    }
    // Right ascending
    foreach ($right as $req) {
        $currentPosition = $req;
        $visitOrder[]    = $req;
    }
...
function clookAlgorithmString($requests, $head, $diskSize)
{
    $sorted = bubbleSort($requests);
    $upper  = [];
    $lower  = [];
    ...
    // Jump to first lower if any
    if (!empty($lower)) {
        $currentPosition = $lower[0]; // not added to path unless you want
        $visitOrder[]    = $lower[0];
        for ($i = 1; $i < count($lower); $i++) {
            $currentPosition = $lower[$i];
            $visitOrder[]    = $lower[$i];
        }
    }
