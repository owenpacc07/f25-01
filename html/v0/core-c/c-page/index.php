...
//$coreCPath = "/var/www/projects/s25-01/html/files/core-c/c-page";
$coreCPath = realpath(__DIR__ . '/../../../files/core-c/c-page/');
$inputFile = "$coreCPath/in-page.dat"; // Input file 
...
<head>
	...
	function randomizePageInputs () {
     	    const REF_LENGTH   = 20;   // how many references
     	    const PAGE_MIN     = 0;    // lowest page number
     	    const PAGE_MAX     = 9;    // highest page number

     	    // build an array of random ints in [PAGE_MIN, PAGE_MAX]
     	    const refs = Array.from({length: REF_LENGTH}, () =>
                 Math.floor(Math.random() * (PAGE_MAX - PAGE_MIN + 1)) + PAGE_MIN);

     	    // write the comma‑separated list into the text box
     	    document.getElementById('pageInput').value = refs.join(', ');
        }
...
<body>
                    ...
                    <div class="card-body">
                        ...
                        <form method="post" class="text-center">
                            ...
                            <button type="button" class="btn btn-purple d-inline mr-2" onclick="showChart(event)">Show
                                Chart</button>
			    <button type="button" class="btn btn-purple d-inline mr-2" onclick="randomizePageInputs()">
    ...
    <?php
    function writeInput($pages) {
        // -- Write input to respective input dat file --

        $pCount = count($pages);

        $file = fopen("../../../files/core-c/c-page/in-page.dat", "w");
        ...
        fwrite($file, $pages[$pCount - 1]); // Write the last page without a comma
        fclose($file);
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['compare'])) {
        ...
        $mechanisms =['021', '022', '023', '024', '025']; // Mechanism IDs
	...
	file_put_contents("../../../files/core-c/c-page/in-page.dat", $_POST['pages']);


	function makeLine($page, $frames, $fCount, $fault) {
    	   $cells = array_pad($frames, $fCount, '-');
    	   array_unshift($cells, $page); // add page at the front
    	   $cells[] = $fault;            // add fault (0 or 1) at the end
    	   return implode(',', $cells);
	}

/*
	// Output-generating algorithm methods
   function FIFOoutput($pages) { return fifoSim($pages, 3)[0]; }
   function OPToutput ($pages) { return optSim ($pages, 3)[0]; }
   function LRUoutput ($pages) { return lruSim ($pages, 3)[0]; }
   function LFUoutput ($pages) { return lfuSim ($pages, 3)[0]; }
   function MFUoutput ($pages) { return mfuSim ($pages, 3)[0]; }

   // Store to mechanism output files
   $outputFuncs = [
       '021' => 'FIFOoutput',
       '022' => 'LRUoutput',
       '023' => 'OPToutput',
   ...
   foreach ($outputFuncs as $mid => $func) {
   	$path = realpath("../../../files/core-c/m-$mid/out-$mid.dat");
    	file_put_contents($path, $func($pages));
   }
...
function FIFOoutput($pages) {
    ...
    foreach ($pages as $p) {
        $fault = !in_array($p, $frames);
        if ($fault) {
            if (count($frames) == $f) array_shift($frames);
            $frames[] = $p;
            $faults++;
        }
        $rows[] = makeLine($p, $frames, $f, $fault);
    }
    ...
    foreach ($pages as $i => $p) {
        ...
        if ($fault) {
            if (count($frames) < $f) {
                ...
                $idx = -1;
                $dist = -1;
                foreach ($frames as $k => $pg) {
                    $next = array_search($pg, array_slice($pages, $i + 1), true);
                    $next = ($next === false) ? PHP_INT_MAX : $next;
                    if ($next > $dist) {
                        $dist = $next;
                        $idx = $k;
                    }
    ...
    foreach ($pages as $i => $p) {
        ...
        if ($fault) {
            if (count($frames) < $f) {
                ...
                asort($used);
                $victim = key($used);
                $idx = array_search($victim, $frames, true);
                $frames[$idx] = $p;
                unset($used[$victim]);
            }
            $faults++;
    ...
    foreach ($pages as $p) {
        ...
        if ($fault) {
            if (count($frames) < $f) {
                ...
                $least = min($freq);
                $victim = array_keys($freq, $least, true)[0];
                $idx = array_search($victim, $frames, true);
                $frames[$idx] = $p;
                unset($freq[$victim]);
            }
    ...
    foreach ($pages as $p) {
        ...
        if ($fault) {
            if (count($frames) < $f) {
                ...
                $most = max($freq);
                $victim = array_keys($freq, $most, true)[0];
                $idx = array_search($victim, $frames, true);
                $frames[$idx] = $p;
                unset($freq[$victim]);
            }
