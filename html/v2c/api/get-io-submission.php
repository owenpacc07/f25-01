...
if ($method == 'GET') {
	if (isset($_GET['mechanism'])) {
		...
		$inputContents = fread($fileInput, filesize(ROOT_DIR . "/files/core-s/m-$mID/in-$mID.dat"));

		$fileOutput = fopen(ROOT_DIR . "/files/core-a/m-$mID/out-$mID.dat", "r") or die(json_encode(array("error" => "Output File not found")));
		$outputContents = fread($fileOutput, filesize(ROOT_DIR . "/files/core-s/m-$mID/out-$mID.dat"));
