...
export async function loadData(user_id, experiment_id, family_id, mechanism_id) {
    ...
    if (response2.output === "No output generated for m001") {
        console.error("No output generated for mechanism m001. Check server-side logic or output file (out-001.dat).");
        return false;
    }
...
function parseOutputDataFile(output, section) {
    ...
    console.log("Parsing output lines for section", section, ":", lines);

    if (lines.length === 0) {
        console.warn("No output lines to parse.");
    ...
    lines.forEach((line, index) => {
        ...
        if (inStartEndSection) {
            ...
            if (burst.length === 3 && !isNaN(burst[0]) && !isNaN(burst[1]) && !isNaN(burst[2])) {
                ...
                console.warn(`Failed to parse start/end line: ${line}`);
            }
            ...
            if (parts.length >= 11) {
                ...
                console.warn(`Skipped detailed table line (insufficient parts): ${line}`);
            }
        }
    });
