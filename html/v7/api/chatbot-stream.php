...
function constructSystemPrompt($mechanism_id, $mode, $context) {
    // Layer 1: Global Instructions with Formatting Guidelines
    $layer1 = "You are an educational assistant for OS Visuals, a platform for learning Operating Systems algorithms. " .
        ...
        "\n\n" .
        "RESPONSE FORMAT: Use clean, plain text format. Avoid markdown formatting like **bold** or *italic*. " .
        "Use numbered lists (1., 2., 3.) for steps and bullet points with line breaks for readability. " .
        "Create clear paragraph breaks between topics. Keep explanations concise and focused on the student's question.";

    // Layer 2: Category Knowledge
    $layer2 = getCategoryKnowledge($mechanism_id);

    // Layer 3: Mechanism-Specific Context (now includes actual data)
...
/**
 * Get mechanism-specific context with actual mechanism data
 */
function getMechanismContext($mechanism_id, $mode, $context) {
    ...
    // NEW: Add actual mechanism data if available
    // This data comes from chatbot-data-extractor.js and contains the user's actual input/output
    if (!empty($context['mechanismData']) && !empty($context['mechanismData']['dataContext'])) {
        $context_text .= "\n\n" . $context['mechanismData']['dataContext'];
    } elseif (!empty($context['dataContext'])) {
        // Fallback: raw data context
        $context_text .= "\n\n" . $context['dataContext'];
    }

    // Add current animation state if available
    if (!empty($context['currentStep']) || !empty($context['animationState'])) {
        $context_text .= "\n\nCurrent state:";
        if (!empty($context['currentStep'])) {
            $context_text .= " Step " . $context['currentStep'];
        }
        if (!empty($context['totalSteps'])) {
            $context_text .= "/" . $context['totalSteps'];
        }
        if (!empty($context['animationState'])) {
            if (is_array($context['animationState'])) {
                if ($context['animationState']['isPlaying']) {
                    $context_text .= " (playing)";
                } else {
                    $context_text .= " (paused)";
                }
            }
