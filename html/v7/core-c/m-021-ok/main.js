...
/*await loadData().then((data) => {
    if (!data) {
      document.getElementById("text").innerText = "Could not load output data (Document))";
      throw new Error("Could not load output data (throw)");
    } else {
  ...
  .catch((err) => {
    ...
    document.getElementById("text").innerText = "Could not load output data (It is in the catch error)";
  });
*/
...
let playButton2 = document.getElementById("Vizualize");
...
let endButton2 = document.getElementById("Run");
let faultCount = 0;
...
function refreshAnim() {
  ...
  playButton2.innerText = "Vizualize";
...
function playAnim2() {
  if (timeInterval) { // pause
    ...
    changeAnimState("mid-auto"); 
    playButton2.innerText = "Vizualizing";
    timeInterval = setInterval(nextAnim, 1000);
  }
}
...
function changeAnimState(state) {
  ...
  switch (state) {
    ...
    case "end":
      ...
      $('#run').prop("disabled", true).text("Run");
    ...
    case "mid-auto":
      ...
      $('#run').prop("disabled", false).text("Pause");
      $('#stepForward').prop("disabled", true);
      $('#skipForward').prop("disabled", false);
      $('#end').prop("disabled", false);
      break;
