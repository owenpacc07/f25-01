...
*/
public class m005 {
	private static String filesDirectory = "../../../files/core-c";
	...
	static void updateFlagFile() throws IOException{
		try {
			File flagFile = new File(filesDirectory + "/m-005/flag-file.txt");
	...
	public static void inFile() throws FileNotFoundException {
		File inputFile = new File(filesDirectory + "/c-cpu/in-cpu.dat");
		Scanner scan = new Scanner(inputFile);
	
		typeScheduler = 4; // Set for Round Robin
		quantum = 3; // Hardcoded quantum time
	
		// Iterate through each line of the input file
		while (scan.hasNextLine()) {
			// Split the current line into parts using comma as the delimiter
			String currentProcessInfo[] = scan.nextLine().replace(" ", "").split(","); // Remove spaces and split on commas
			...
			for (int i = 0; i < currentProcess.length; i++) {
				currentProcess[i] = Integer.parseInt(currentProcessInfo[i].trim()); // Trim any residual whitespace
			}
	
			// Creates a new Process and puts it into process array
			Process newProcess = new Process(currentProcess[0], currentProcess[1], currentProcess[2], currentProcess[3]);
			process.add(newProcess);
	
			// Clean up (optional, as these will be garbage collected)
	...
	public static void outFile() throws IOException {
		File outFile = new File(filesDirectory + "/m-005/out-005.dat");
