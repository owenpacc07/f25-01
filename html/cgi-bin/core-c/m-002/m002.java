...
public class m002 {
	...
	static void updateFlagFile() throws IOException {
		// File("/var/www/projects/s23-01/html/files/core/m-002/flag-file.txt");
		File flagFile = new File("../../../files/core-c/m-002/flag-file.txt");
		// File flagFile = new
		// File("/Applications/XAMPP/htdocs/vizos/html/files/core/m-002/flag-file.txt");
	...
	public static void inFile() throws FileNotFoundException {
        try {
            ...
            line = in.nextLine();
            buffer.append(line.split(",")[0]); // Take "1" before the first comma
            typeScheduler = Integer.parseInt(buffer.toString().trim()); // Trim whitespace
            buffer.setLength(0);
    
            // Parse first line as a process
            i = 0;
            while (in.hasNextLine() || i == 0) {
                if (i > 0) line = in.nextLine(); // Get next line after first iteration
                i = 0;
                // Process ID
                ...
                pID = Integer.parseInt(buffer.toString().trim()); // Trim whitespace
                buffer.setLength(0);
                i++; // Skip comma
                ...
                pArrive = Integer.parseInt(buffer.toString().trim()); // Trim whitespace
                buffer.setLength(0);
                i++; // Skip comma
                ...
                pBurst = Integer.parseInt(buffer.toString().trim()); // Trim whitespace
                buffer.setLength(0);
                i++; // Skip comma
    
                // Priority (until end of line)
                while (i < line.length()) {
                    buffer.append(line.charAt(i));
                    i++;
                }
                pPriority = Integer.parseInt(buffer.toString().trim()); // Trim whitespace
                buffer.setLength(0);
    
                Process e = new Process(pID, pArrive, pBurst, pPriority);
                process.add(e);
                i = 1; // Flag to read next line
            }
            in.close();
	...
	public static void outFile() throws IOException {
		File outputFile = new File("../../../files/core-c/m-002/out-002.dat");
		try {
