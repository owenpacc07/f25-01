...
public class m005 {
	...
	public static void CPU() {
		...
		while (endFlag != 1 && cpuTime < 10000) {
			...
			if (typeScheduler == 4) // RR
			{
				...
				if (stopCheck >= size && processRRnoSort.isEmpty() && p.getBurst() == 0) // if all processes are accounted for and queue is empty
					endFlag = 1;
	...
	public static void inFile(String inputFile) throws FileNotFoundException {
		...
		// Hardcode scheduler type for Round Robin (m-005 always uses type 4 with quantum 3)
		typeScheduler = 4;
		quantum = 3;
