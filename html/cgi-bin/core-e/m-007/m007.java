...
public class m007 {
	...
	public static void addToQueue() { // adds a process to a queue if the cpuTime meets arrival time
		for (int i = 0; i < process.size(); i++) {
			if (typeScheduler == 6 && process.get(i).getArrive() == cpuTime) {
			...
			if (typeScheduler == 6) { // P-PH
				if (p.getBurst() == 0 ||
						!processHLsort.isEmpty() &&
								processHLsort.peek().getPri() > p.getPri()) { // if burst has ended or higher priority arrives
				...
				if (stopCheck >= size && processHLsort.isEmpty() && p.getBurst() == 0) // if all processes are accounted for and queue is empty
					endFlag = 1;
	...
	public static void inFile(String inputFile) throws FileNotFoundException {
		...
		// Hardcode scheduler type for Preemptive Priority High (m-007 always uses type 6)
		typeScheduler = 6;
	...
	public static void outFile() throws IOException {
		...
		String scheduler = "Priority High -> Low(Preemptive)";
