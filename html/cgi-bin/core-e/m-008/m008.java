...
public class m008 {
	...
	public static void addToQueue() { // adds a process to a queue if the cpuTime meets arrival time
		for (int i = 0; i < process.size(); i++) {
			if (typeScheduler == 7 && process.get(i).getArrive() == cpuTime) {
			...
			if (typeScheduler == 7) { // P-PL
				if (p.getBurst() == 0 ||
						!processLHsort.isEmpty() &&
								processLHsort.peek().getPri() < p.getPri()) { // if burst has ended or lower priority arrives
				...
				if (stopCheck >= size && processLHsort.isEmpty() && p.getBurst() == 0) { // if all processes are accounted for and queue is empty
					endFlag = 1;
				}
	...
	public static void inFile(String inputFile) throws FileNotFoundException {
		...
		// Hardcode scheduler type for Preemptive Priority Low (m-008 always uses type 7)
		typeScheduler = 7;
	...
	public static void outFile() throws IOException {
		...
		String scheduler = "Priority Low -> High(Preemptive)";
