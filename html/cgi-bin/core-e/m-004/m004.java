...
public class m004 {
	...
	public static void addToQueue() // adds a process to a queue if the cpuTime meets arrival time
	{
		for (int i = 0; i < process.size(); i++) {
			if (process.get(i).getArrive() == cpuTime) {
				processRRsort.add(process.get(i));
				stopCheck++;
			}
		}
	}

	public static Process swap(Process p) // swaps process with one waiting in queue (Preemptive)
	{
		if (p.getBurst() > 0) // if the process is not finished, put it back in the queue
			processRRsort.add(p);

		return processRRsort.remove();
	}
	...
	public static void CPU() {
		...
		// Initial Process
		if (!processRRsort.isEmpty())
			p = processRRsort.remove();
		else
		...
		while (endFlag != 1 && cpuTime < 10000) {
			...
			{
				if (p.getBurst() == 0 || quantumCounter >= quantum) // if burst has ended or quantum time is up
				{
				...
				if (stopCheck >= size && processRRsort.isEmpty() && p.getBurst() == 0) // if all processes are accounted for and queue is empty
					endFlag = 1;
	...
	public static void outFile() throws IOException {
		...
		String scheduler = "Round Robin(Preemptive)";
