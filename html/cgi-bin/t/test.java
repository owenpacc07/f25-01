...
-Preemptive-
4 RR: Round Robin (MUST put the QUANTUM time after the # of processes on 1st row in INPUT data file "in.txt")
...
private static int systime; //system time
private static int[][] sysb = new int[20][50];
private static int[][] sysw = new int[20][50];
private static String[][] sys = new String[20][50];
...
//***********************************************************************************************
			...
			sysb[cpuTime][cpid] = p.getBurst() ; 
                        sysb[cpuTime][5] = cpuTime;

			if (cpuTime!=0)
			for(int k=0; k < pn; k++)
				{
		          	   if (k!=cpid) { sysb[cpuTime][k] = sysb[cpuTime-1][k]; }
                                   else sysb[cpuTime][k] = p.getBurst() ; 
				}
...
/*
			...
			for(int k=0; k < pn; k++)
				{
		          		sysb[cpuTime][k] = sysb[cpuTime-1][k]; 
				}
...
//***********************************************************************************************/
	...
	public static void outFile() throws IOException
	{
		...
		for(int i=0; i < sProcess.size(); i++)	//loop to write processes to file
				out.println("ProcessID: " + sProcess.get(i).getID() + "	|" +
							"Start Time: " + (sProcess.get(i).getStart()) + "	|" +
							"End Time:" + (sProcess.get(i).getEnd()));
		...
		for(int i=0; i < sProcess.size(); i++)
		{
		    	...
		    	out.print("ArriveTime P(" + pid + ")=" + atime );  
		    	out.print("  LastEndTime P(" + pid + ")=" + plastend[pid-1] ); 
		    	out.print("  StartTime P(" + pid + ")=" + stime ); 
		    	...
		    	out.println("  WaittingTime P(" + pid + ")=" + wtime ); 	   
			wait = wait + wtime;
			...
			float avgWait = wait/process.size();
                ...
                // print out DETAILS step by step
		out.println();
		out.println("DETAILS (step by step):");
		out.println("-------------------------------------------------------------------");

                int columns = 2 + 2*process.size();
                int rows = systime; 
