...
public class Main {
	...
	public static void CPU() {
		...
		for (Process proc : process) {
			sortedProcesses.add(proc);
		}
		
		// Simple bubble sort by arrival time
		for (int i = 0; i < sortedProcesses.size() - 1; i++) {
			for (int j = 0; j < sortedProcesses.size() - i - 1; j++) {
				if (sortedProcesses.get(j).getArrive() > sortedProcesses.get(j + 1).getArrive()) {
		...
		for (Process proc : sortedProcesses) {
			...
			if (currentTime < proc.getArrive()) {
				currentTime = proc.getArrive();
			}
			
			int startTime = currentTime;
			int endTime = currentTime + proc.getBurst();
			
			EndProcess endProc = new EndProcess(proc.getID(), startTime, endTime);
			sProcess.add(endProc);
			
			currentTime = endTime;
			
			System.out.println("Process " + proc.getID() + ": arrives=" + proc.getArrive() + 
			                  ", starts=" + startTime + ", ends=" + endTime + ", burst=" + proc.getBurst());
		}
	}
