...
define('ROOT_DIR', '/var/www/p/f25-01/html');

// Get mechanism title
if ($mid) {
    $sql = "SELECT algorithm FROM mechanisms WHERE client_code = ?";
    if ($stmt = mysqli_prepare($link, $sql)) {
        ...
        error_log("Failed to query mechanisms table for mid $mid: " . mysqli_error($link));
    }
}
...
if ($output) {
    error_log("Java m003 output: " . $output, 3, "/var/log/apache2/m003.log");
}

// Check flag-file.txt and out-003.dat
...
if (file_exists($flag_file) && file_get_contents($flag_file) === "1" && file_exists($output_file)) {
    ...
    error_log("Execution failed: flag_file=$flag_file, output_file=$output_file", 3, "/var/log/apache2/m003.log");
    // Optionally write error to out-003.dat for debugging
    file_put_contents($output_file, "No output generated for m003");
}
?>
<!DOCTYPE html>
<html lang="en">
