...
await loadData().then((data) => {
	...
	//Check to see if the table can render the appropriate number of input slots by checking the size
	//Create a var to hold the number of blocks
	var totalNumberOfBlocks = 0;

	//Check every file that we have
	for(file in files) {
	//The 2nd position in the file data is the "total size" of the process
	  totalNumberOfBlocks += file[2];
	}

	//if there are more blocks than there are slots, then the animation won't render correctly
	if(totalNumberOfBlocks > slotcount) {
	//in that case, the number of slots should be adjusted to be equal to the number of blocks
...
function animate(type) {
    ...
    // this line will display the directory table step-by-step
    // need to comment out the displayFullDirectory line though
