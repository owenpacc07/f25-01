...
// const HardCodedInput = [5,1,2,1,6,5,1,3];
...
function nextAnim() {
  ...
  for (let i = 0; i < prevFrames.length; i++) {
    ...
    infoSteps.innerText = (curData.neededIn[i] == '+') ? '(never)' : curData.neededIn[i];
    if (i == replaceIndex) {
...
function changeAnimState(state) {
  ...
  switch (state) {
    ...
    case "mid-auto":
      ...
      $('#run').prop("disabled", false).text("Stop");
      $('#stepForward').prop("disabled", true);
      $('#skipForward').prop("disabled", false);
      break;
  }
