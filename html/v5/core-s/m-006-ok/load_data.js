...
/* OLD WAY OF PARSING OUT FILE
function parseOutputDataFile() {
    let outputData = [];
    let allText = loadDataFile(output_file_path);
    let ganttVals = allText.split('\n').slice(3,10); // vals for gantt chart
...
function parseExtraOutputDataFile() {
    let extraOutputData = [];
    let allText = loadDataFile(output_file_path);
    let extraVals = allText.split('\n').slice(11,28); 
    extraVals.forEach((line) => {
        // parse line, convert to nums
        let burst = line.split(',').map(String);
        if (burst.length > 0)
            extraOutputData.push({
                time: burst[0],
                p1RemaingBurst: burst[1],
                p2RemaingBurst: burst[2],
                p3RemaingBurst: burst[3],
