...
//page replacment optimal
// Input and output file paths

//contributors: Jaedan Smith
//i decided to keep most of the file interpetion code as it seems to be crucial for site operation, anytime they are deleted or altered, the site wont load at all
//even when deleting the code requirements
...
5,3,1,5,1,2,1,`+`,``,
1,3,1,5,0,1,0,`+`,``,
3,3,1,5,0,0,`+`,`+`,``,5];//output, Origonally i kept the # of allocated frames at the first value, but that messed up my implentation
//format of output explained: `-` is for "used in" values, `+` is for frames never allocated again
//first # in output is # of frames
//empty string = end of result, Last slot in array is # of faults
const HardCodedInput = [3,1,2,1,6,5,1,3];//input
...
async function fetchPHP(type) {
    await fetch(`https://cs.newpaltz.edu/p/s23-01/v3/core/m-${mid}/manage-flag-file.php`, {//changing the file to s23 prevents data from loading
        method: 'POST',
        headers: {
...
function parseInputDataFile() {
   ...
   //let allval = HardCodedInput; //hardcoded values
    ...
    //previous code hrd coding input
    // let testing = [];
    // allText.split(',').forEach((val) => {
    //     let testpage = parseInt(val);
    //     if (testpage >= 0) { // make sure page is valid
    //         testing.push(testpage);
    //     }
    // });
    // console.log("Test File Input " + testing);
    // console.log("Test HC Input " + input);
...
function parseOutputDataFile() {
    ...
    for(let i = 0; i <= data.input.length; i++) { //start at i = 0 because First value is no longer at # of frames
        ...
        if (result.length-1 != (frameCount*2+2)) { // needed to make sure there is an appropiate amount of elements result contains all elements of frame + used-in vals for all frames + cur page + faulted boolean, 
            ...
            slot.neededIn = [] //for the table at the bottom, "needed in" values 
    ...
    //console.log("ouputlengh",output[0].frames.length);
    return output;
}
