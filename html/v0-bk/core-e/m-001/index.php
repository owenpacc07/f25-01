...
if ($output) {
    error_log("Java m001 output: " . $output, 3, "/var/log/apache2/m001.log");
}

// Check flag-file.txt and out-001.dat
...
if (file_exists($flag_file) && file_get_contents($flag_file) === "1" && file_exists($output_file)) {
    ...
    error_log("Execution failed: flag_file=$flag_file, output_file=$output_file", 3, "/var/log/apache2/m001.log");
    // Optionally write error to out-001.dat for debugging
    file_put_contents($output_file, "No output generated for m001");
}
?>
